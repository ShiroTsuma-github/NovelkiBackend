...
public class GenreFeatureTests
{
    ...
    public async Task CreateGenre_ShouldPersistAndReturnDto()
    {
        ...
        var result = await handler.Handle(new CreateGenreCommand("Fantasy", "Magic stories"), CancellationToken.None);
    ...
    public async Task GetAllGenres_ShouldReturnPaginatedResult()
    {
        ...
        PaginatedResult<GenreDto> result = await handler.Handle(new GetAllGenresQuery(Skip: 1, Take: 1), CancellationToken.None);
    ...
    private sealed class FakeGenreRepository : IGenreRepository
    {
        ...
        {
            return Task.FromResult(_genres.FirstOrDefault(g => string.Equals(g.Name, name.Trim(), StringComparison.OrdinalIgnoreCase)));
        }
